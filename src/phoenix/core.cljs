...
(def zoom-screen-share? (atom false))
...
(defn to-middle
  ...
  (when-let [window (.focused js/Window)]
    (let [screen-frame (.flippedVisibleFrame (.screen window))]
      (.setFrame window
                 #js
                  {:x (+ (.-x screen-frame) (* 0.25 (.-width screen-frame)))
                   :y (.-y screen-frame)
...
(defn move-to-screen
  ...
  (when (and window screen)
    (let [window-frame (.frame window)
      ...
      (.setFrame window
                 #js
                  {:width (round (* x-ratio (.-width window-frame)))
...
(defn focus-or-start
  ...
  (if-let [app (.get js/App title)]
    (let [current-window-hash (some-> (.focused js/Window)
                                      (.hash))
          windows (.windows app #js {:visible true})]
      (when-let [other-windows (seq (filter (fn [w]
                                              (not= current-window-hash (.hash w)))
                                            windows))]
...
(bind "z" ["alt" "cmd" "ctrl"] (fn [] (swap! zoom-screen-share? not)))
